...
void putch(unsigned char byte) 
{
	...
	while(!TXIF)  //TXIF but used to check if RXREG is full or empty , if 0 -> full, 1 -> empty
	{
		continue;
	}
	TXIF = 0;
...
{
	while(*s)	     //run till null character is found	
	{
		putch(*s++);	//copu byte by byte
	}
	return 0;
}
...
unsigned char getch(void)
{
	...
	while(!RCIF) //RCIF but used to check if RCREG is full or empty , if 0 -> empty, 1 -> full
	{
		continue;
	}
	RCIF = 0;
...
unsigned char getche(void)
{
	...
	putch(c = getch()); //1st receive char from laptop to mc(getch)
                        //2nd transmit the received char to laptop (putch) 
